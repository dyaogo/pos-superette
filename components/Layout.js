...
import { 
  Home, ShoppingCart, Package, FileText, Users, CreditCard, 
  Settings, Menu, X, ClipboardList, Store as StoreIcon 
} from 'lucide-react';
...
export default function Layout({ children }) {
  ...
  const menuItems = [
    ...
    { path: '/physical-inventory', icon: ClipboardList, label: 'Inventaire Physique' },
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--color-bg)' }}>
      ...
      <aside style={{
        ...
        <div style={{
          ...
          {sidebarOpen && (
            <h2 style={{ margin: 0, color: 'var(--color-primary)' }}>POS Superette</h2>
          )}
          <button
      ...
      <main style={{
        ...
        <header style={{
          ...
          <div style={{ fontSize: '18px', fontWeight: '600', color: 'var(--color-text-primary)' }}>
            {menuItems.find(item => item.path === router.pathname)?.label || 'POS Superette'}
          </div>
