...
const BAUD_KEY       = 'display_baud_rate';

function getDisplayBaud() {
  try { return parseInt(localStorage.getItem(BAUD_KEY) || '9600', 10); } catch { return 9600; }
}
...
async function ensureDisplayPort() {
  ...
  await port.open({ baudRate: getDisplayBaud(), dataBits: 8, stopBits: 1, parity: 'none' });
  savePortInfo(port);
  _displayPort = port;
  return port;
}
...
/**
 * Ferme le port, change le baud rate, rouvre, envoie 1500.
 * Utilisé par le panneau de diagnostic pour trouver la bonne vitesse.
 */
export async function testDisplayBaud(baud) {
  // Fermer le port actuel
  if (_displayPort) { try { await _displayPort.close(); } catch {} _displayPort = null; }
  // Mémoriser le nouveau baud rate
  try { localStorage.setItem(BAUD_KEY, String(baud)); } catch {}
  // Rouvrir avec le nouveau baud rate (réutilise le port sauvegardé si possible)
  const savedInfo = getSavedPortInfo();
  let port = null;
  if (savedInfo) {
    const ports = await navigator.serial.getPorts();
    for (const p of ports) {
      ...
      // COM natif → pas de VID/PID → on prend le premier port disponible
      if (ports.length === 1 || (savedInfo.usbVendorId && info.usbVendorId === savedInfo.usbVendorId)) {
        port = p;
        break;
      }
    }
  ...
  savePortInfo(port);
  _displayPort = port;
  // Envoyer "1500\r" comme test
  const bytes = Uint8Array.from('1500\r', c => c.charCodeAt(0));
  const writer = port.writable.getWriter();
