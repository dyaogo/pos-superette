...
async function ensureDisplayPort() {
  ...
  if (savedInfo) {
    ...
    if (savedInfo.usbVendorId) {
      // Port USB : reconnexion par VID/PID
      for (const port of ports) {
        const info = port.getInfo ? port.getInfo() : {};
        if (info.usbVendorId === savedInfo.usbVendorId && info.usbProductId === savedInfo.usbProductId) {
          try {
      ...
      // Port natif Windows (ex. COM2) — pas de VID/PID, on prend le premier port accordé
      const port = ports[0];
      try {
        await port.open({ baudRate: DISPLAY_BAUD, dataBits: 8, stopBits: 1, parity: 'none' });
      } catch (err) {
